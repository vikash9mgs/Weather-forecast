...
def suscribe(request):
    ...
    return render(request, "weather_api/home.html", {"msg": 'Thanks for Subscribe'})
...
def mynews(request):
    ndata = mnews.objects.all().order_by("-id")
    nhead = request.POST.get('nhead')
    ntitle = request.POST.get('ntitle')
    ncity = request.POST.get('ncity')
    ndesc = request.POST.get('ndesc')
    npic = request.POST.get('npic')
    mydict = {"nhead": nhead, "ntitle": ntitle, "ncity": ncity, "ndesc": ndesc, "npic": npic}

    return render(request, 'weather_api/news.html', context=mydict)


def ndetails(request):
    return render(request, 'weather_api/ndetails.html')
